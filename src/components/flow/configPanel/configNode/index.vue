  ...
  <a-card class="box" title="节点设置">
    <a-form :label-col="{ span: 7 }" :wrapper-col="{ span: 16 }">
      ...
      <a-form-item label="添加节点" v-if="addNodeIsShow">
        <a-button type="primary" @click="addNode" style="margin-left: -115px;">
          <a-icon type="plus" />
        </a-button>
      </a-form-item>
      <a-form-item
        ...
        :label="item.type"
      >
        <a-input v-model="item.textWrap.text" v-if="!item.deleteNodeIsShow"></a-input>
        <a-input
          v-model="item.textWrap.text"
          v-if="item.deleteNodeIsShow"
          style="width:65%;margin-right: 10px;"
        ></a-input>
        <a-button type="danger" v-if="item.deleteNodeIsShow" @click="deleteNode(index)">
          <a-icon type="minus" />
        </a-button>
      </a-form-item>
    </a-form>
  ...
  methods: {
    ...
    nodeOpt(id) {
      ...
      if (id) {
        ...
        this.cellNew1.index = this.cellNew2.length / 2;//当前节点数
        this.globalNodeAttr.attrsFontSize = this.cellNew1.commonStyle1.fontSize;//字号
        this.globalNodeAttr.attrsBackgroundColor = this.cellNew1.commonStyle.stroke;//背景色
    ...
    // 节点背景颜色点击
    colorClick(item) {
      console.log(this.globalNodeAttr)
